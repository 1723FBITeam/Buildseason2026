...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        driverXbox.x().onTrue(Commands.runOnce(() -> servoSubsystem.levelOne()));
        driverXbox.y().onTrue(Commands.runOnce(() -> servoSubsystem.LevelTwo()));
        driverXbox.b().onTrue(Commands.runOnce(() -> servoSubsystem.LevelThree()));

        driverXbox.leftBumper().whileTrue(Commands.run(() -> elevatorSubsystem.moveUp(), elevatorSubsystem))
                .onFalse(Commands.runOnce(() -> elevatorSubsystem.stop()));
        driverXbox.rightBumper().whileTrue(Commands.run(() -> elevatorSubsystem.moveDown(), elevatorSubsystem))
                .onFalse(Commands.runOnce(() -> elevatorSubsystem.stop()));
    }
